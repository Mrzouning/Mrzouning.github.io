...
2. 在首页 `index.html` 增加文章卡片和分类信息。
3. 更新 `archives/index.html`、年度归档和月份归档。
4. 在文章底部增加上一篇、下一篇或归档入口。
5. 更新 `sitemap.xml` 和 `feed.xml`。
6. 用本地静态服务器检查首页、文章页、归档页和移动端布局。
7. 通过 Git 提交并推送到 `master`，GitHub Pages 会自动发布。

文章建议包含：明确的问题、真实过程、失败记录、可复用结论和边界说明。涉及模型能力时，注明测试日期，避免把快速变化的结论写成永久事实。
